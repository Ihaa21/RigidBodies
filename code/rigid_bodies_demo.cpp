...
DEMO_INIT(Init)
{
    ...
    // NOTE: Create render data
    {
        ...
        // NOTE: Create PSO
        {
            ...
            VkPipelineInputAssemblyAdd(&Builder, VK_PRIMITIVE_TOPOLOGY_LINE_STRIP, VK_FALSE);
            VkPipelineRasterizationSetLineWidth(&Builder, 3.0f);
    ...
    VkCommandsBegin(Commands, RenderState->Device);
    {
        ...
        // NOTE: Push circle mesh
        {
            ...
            for (u32 Id = 0; Id < Mesh.NumVertices - 2; ++Id)
            {
                f32 Angle = (2.0f * Pi32 / f32(Mesh.NumVertices)) * f32(Id);
                GpuMemory[Id] = 0.5f*V3(Cos(Angle), Sin(Angle), 0.0f);
            }
            GpuMemory[Mesh.NumVertices - 2] = GpuMemory[0];
            GpuMemory[Mesh.NumVertices - 1] = V3(0);
...
{
    {
        ...
        RenderTargetPassBegin(&DemoState->RenderTarget, Commands, RenderTargetRenderPass_SetViewPort | RenderTargetRenderPass_SetScissor);
        {
            ...
            for (; InstanceId < Scene->NumOpaqueInstances; )
            {
                ...
                vkCmdDraw(Commands->Buffer, CurrMesh->NumVertices, NextInstanceId - InstanceId, 0, InstanceId);
